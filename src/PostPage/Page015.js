...
const Page016 = () => {
  ...
  const onFinish = async (values) => {
    try {
      const response = await axios.post('http://localhost:5000/api/fcmr15/post', values);
            ...
            <Form.Item label="項目:" name="Item">
              <Radio.Group value={value} onChange={onChange}>
                <Space direction="vertical">
                  <Radio value={"育苗場所"}>育苗場所</Radio>
                  <Radio value={"溫/網室"}>溫/網室</Radio>
                  <Radio value={"資材放置場所"}>資材放置場所</Radio>
                  <Radio value={"倉庫與工具間"}>倉庫與工具間</Radio>
                  <Radio value={"集貨場"}>集貨場</Radio>
                  <Radio value={"包裝場"}>包裝場</Radio>
                  <Radio value={"其他"}>其他
                    {valueA === "其他" ? (
